...
void w<T>(T valueToSHow) { Console.WriteLine(valueToSHow); }
/*
var X = new string[] { "be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe",
...
{
    var s = input[i].Split(' ');
    var val = new string[10];
    foreach(var s1 in s)
    {
        if (s1.Length == 2) val[1] = s1;
        if (s1.Length == 4) val[4] = s1;
        if (s1.Length == 3) val[7] = s1;
        if (s1.Length == 7) val[8] = s1;
    }
    foreach(var s1 in s)
    {
        if(s1.Length == 5)
        {
            if (PartOf(s1, val[1])) val[3] = s1;
            else if (PartOf(s1, minus(val[4], val[1]))) val[5] = s1;
            else val[2] = s1;
        }
    }
    foreach(var s1 in s)
    {
        if(s1.Length == 6)
        {
            if (PartOf(s1, val[4])) val[9] = s1;
            else if (PartOf(s1, val[5])) val[6] = s1;
            else val[0] = s1;
        }
    }

    int number = 0;
    foreach(var s1 in output[i].Split(' '))
    {
        int digit = 0;
        for (int k = 0; k<10;k++)
        {
...
bool PartOf(string whole, string part)
{
    bool partof = true;
    foreach(char c in part)
    {
        partof &= contains(whole, c);
    }
    return partof;
}
 bool contains(string whole, char c)
{
    bool found = false;
    foreach (char c1 in whole)
    {
        if (c == c1) found = true;
    }return found;
} 

string minus(string org, string min)
{
    string result = "";
    foreach(char c in org)
    {
        if (!contains(min, c)) result += c;
    }
    return result;
}
...
bool equal (string first, string second)
{
    ...
    foreach (char c in first) if (!contains(second, c)) return false;
    return true;
}