...
void Run()
{
    ...
    */
    ...
    int top(int v0) { return Math.Max(location(v0,v0), location(v0, v0 + 1)); }  // s_top = v0 + 1/2 , see 
    ...
    while ( vx0 <= x2)
    {
        ...
        while ((location(vx0,s) <= x2) || (s >= vx0 && s <= x2 ))
        {
            ...
            while (y <= y2) // else overshoot
            {
                if (y >= y1) // hit
                {
                    maxy = Math.Max(maxy, top(vy0));
                    bullseyes.Add((vx0, vy0));
                }
                vy0++;
                y = location(vy0,s);
            }
