...
void Run(string inputfile, bool isTest, long supposedanswer1, long supposedanswer2)
{
    ...
    var rules = new Dictionary<string, (char v, char op, long val, string target)[]>();
    foreach (var rule in S[0])
    {
        var s1 = rule.Split("{");
        var name = s1[0];
        var s2 = s1[1].Substring(0, s1[1].Length-1).Split(",");
        var ar = new (char v, char op, long val, string target)[s2.Length];
        for (int i=0; i<s2.Length-1;i++)
        {
            var s3 = s2[i].Substring(2).Split(":");
            ar[i] = (s2[i][0], s2[i][1], long.Parse(s3[0]),s3[1]);
        }
    ...
    foreach (var part in S[1]) 
    {
        ...
        while (workflow != "A" && workflow!="R")
        {
            ...
            {
                var w = wf[i];
                var j = w.v == 'x' ? 0 : w.v == 'm' ? 1 : w.v == 'a' ? 2 : 3;
                if (w.op == '<' && s1[j] < w.val || w.op == '>' && s1[j] > w.val || w.op == ' ')
                {
                    workflow = w.target;
                    break;
                }
